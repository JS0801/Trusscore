...
define(['N/error', 'N/log', 'N/record', 'N/search', 'N/format'],
  function (error, log, record, search, format) {
    ...
    // added by sim from SB
    function beforeSubmit(context) {
      var rec = context.newRecord;

      // Get header amount
      var headerAmount = parseFloat(rec.getValue('custbody_ts_vendor_freight')) || 0;
      // Get the header po item
      var headerpoitem = rec.getValue("custbody_tc_po_item");

      if (headerAmount <= 0) return;
      ...
      for (var i = 0; i < lineCount; i++) {
        var qty = parseFloat(rec.getSublistValue({ sublistId: 'line', fieldId: 'custcol_tc_shipping_qty', line: i })) || 0;
        totalQty += qty;
      }
      log.debug('totalQty', totalQty);
      if (totalQty === 0) return;

      // Calculate cost per qty
      var costPerQty = headerAmount / totalQty;
      log.debug('costPerQty', costPerQty);

      // Set cost on each line
      for (var i = 0; i < lineCount; i++) {
        var qty = parseFloat(rec.getSublistValue({ sublistId: 'line', fieldId: 'custcol_tc_shipping_qty', line: i })) || 0;
        var lineCost = qty * costPerQty;

        rec.setSublistValue({
          sublistId: 'line',
          fieldId: 'custcol_trusscore_po_amount',
          line: i,
          value: lineCost.toFixed(2)
        });

        // commented because this logic moved to client script
        // if(headerpoitem){
        //   rec.setSublistValue({
        //     sublistId: 'line',
      ...
      try {
        ...
        if (context.type == context.UserEventType.CREATE || context.type == context.UserEventType.EDIT) {
          ...
          if (isScrapRecord(scraprec)) {
            ...
            for (var r = consolidated_shippingrecord.getLineCount({ sublistId: 'line' }) - 1; r >= 0; r--) {
              consolidated_shippingrecord.removeLine({
                sublistId: 'line',
                line: r,
                ignoreRecalc: false
              });
            ...
            consolidated_shippingrecord.setValue({ fieldId: 'custbody_ds_final_cal_weight', value: 0 });
            consolidated_shippingrecord.setValue({ fieldId: 'custbody_tc_shipping_charge', value: '' });
            consolidated_shippingrecord.setValue({ fieldId: 'custbody_tc_related_tran', value: '' });
            ...
            for (var key in groupedBySalesOrder) {
              ...
              for (var i = 0; i < data.length; i++) {
                ...
                if (i == 0) {
                  ...
                  //added by sim from SB
                  var isShippingChargeChanged = consolidated_shippingrecord.getValue({
                    fieldId: 'custbody_csr_shipping_charge_changed'
                  });

                  if (isShippingChargeChanged !== true && isShippingChargeChanged !== 'T') {
                    consolidated_shippingrecord.setValue({
                      fieldId: 'custbody_tc_shipping_charge',
                      value: getRelatedData.shipping_charge
                    });

                    log.debug({
                      title: 'Shipping Charge Updated From Related Transaction',
                      details: {
                        relatedTransaction: keySO,
                        shippingCharge: getRelatedData.shipping_charge
                      }
                    });
                  } else {
                    log.debug({
                      title: 'Shipping Charge Not Updated - Manual Override Flag Is True',
                      details: {
                        relatedTransaction: keySO,
                        sourceShippingCharge: getRelatedData.shipping_charge
                      }
                    });
                  }
    ...
    function getSalesOrderData(item, line, keySO) {
      ...
      salesorderSearchObj.run().each(function (result) {
        object = {
          ...
          item_hscode: result.getValue({
            name: "custitem_ts_hs_code",
            join: "item",
            label: "HS CODE"
          }),
          item_bol_group: result.getValue({
            name: "custitem_tc_item_bol_group",
            join: "item",
            label: "BOL Group"
          }),
